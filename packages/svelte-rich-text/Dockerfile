...
COPY . .
RUN turbo prune svelte-rich-text --docker
...
RUN NODE_OPTIONS="--max-old-space-size=8192" yarn turbo run build --filter=svelte-rich-text --concurrency 2
...
COPY --from=installer /app/packages/svelte-rich-text/package.json .
COPY --from=installer /app/packages/svelte-rich-text/build /app/build
